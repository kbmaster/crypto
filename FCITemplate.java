...
public class FCITemplate {
    ...
    public FCITemplate(boolean isDF, boolean isEF, String fileName, short fileSize, byte fdb, short fileId, 
byte lifeCycleStatusByte) {
        this.isDF = isDF;
        this.isEF = isEF;
        this.fileName = fileName;
        this.fileSize = fileSize;
 ...
 public void buildFromBuffer(byte[] buffer, int offset, int length) throws Exception {
		...
		this.fileName = asciiDecoder.decode(name).toString();
                break;
            case (byte)0x81:
                //EF
